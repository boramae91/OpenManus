...
ANALYSIS_KEYWORDS = {'종목분석', '기술분석', '재무분석', '뉴스', '컨센서스', '재무제표', '재무비율', '투자지표', '지분분석', '업종분석', '경쟁사비교', '거래소공시', '금감원공시', 'ETF', 'ETN', '리포트', '실적속보', '스크리닝', '랭킹', '캘린더', '신용등급', '기업주소록', '이용안내'}
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if match:
                    ...
                    # 분석 키워드 앞까지(공백 포함) 추출
                    m3 = re.search(r'^([가-힣A-Za-z0-9 ]+?)(종목 ?분석|기술 ?분석|재무 ?분석|뉴스|분석|\\d{4})', query)
                    if m3:
...
def save_interaction_log(prompt, response, steps=None, meta=None, memory=None, log_path=None):
