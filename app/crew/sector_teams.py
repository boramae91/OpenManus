...
당신의 모든 응답은 반드시 다음 3단계 구조를 정확히 따라야 합니다:
=== 1단계: Self-Ask with ToT ===
=== 2단계: ReAct ===
=== 3단계: CoT Reasoning + Self-Critique ===

"### 1. 수익성 분석" 같은 기존 형식은 절대 사용 금지입니다.
...
당신의 응답은 반드시 다음 템플릿을 정확히 따라야 합니다:

```
=== 1단계: Self-Ask with ToT (섹터별 맞춤 질문 구성 및 사고 분기) ===

🌳 Tree of Thoughts 기법으로 핵심 질문들을 체계적으로 구성:

[여기에 Q1-Q5 질문 구성과 우선순위 설정]

=== 2단계: ReAct (Reason + Action) - 검색 및 정보 수집 ===

🔍 각 질문에 대한 체계적 정보 수집과 추론 수행:

[여기에 각 질문별 Reason → Action → Observation 사이클]

=== 3단계: CoT Reasoning + Self-Critique (최종 분석 및 자기 검증) ===

🧠 수집된 모든 정보를 바탕으로 체계적 추론 수행:

[여기에 Chain of Thought 추론 + Self-Critique 검증 + 최종 투자 의견]
```

{output_format}

🚨 **절대적 필수 사항 (위반 시 분석 실패로 간주)**:
1. 위 템플릿 구조를 정확히 사용할 것 (=== 1단계 ===, === 2단계 ===, === 3단계 === 필수)
2. 각 단계를 건너뛰지 말 것
3. "### 1. 수익성 분석" 같은 기존 형식 사용 금지
4. 반드시 Q5 섹터 특화 질문을 1단계에 포함할 것
5. 3단계에서 Self-Critique는 필수 포함

🔥 **최종 확인**: 응답을 시작하기 전에 반드시 "=== 1단계:" 로 시작하는지 확인하세요!
""",
                    ),
                    MessagesPlaceholder(variable_name="chat_history"),
                    ("human", "{input}"),
