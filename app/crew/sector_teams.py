    ...
    def create_sector_team(self, sector: GICSSector) -> SectorTeam:
        ...
        fundamental_analyst = AnalystAgent(
            ...
            + [
                ...
                "3. Free Cash Flow 산출 및 FCF Yield 계산식 명시 - 제공된 현금흐름표 실제 데이터 사용",
                "4. Working Capital 변동이 영업현금흐름에 미치는 영향 정량화 - 제공된 재무제표 실제 수치 활용",
                "5. 부채상환능력 지표 (Interest Coverage, Debt Service Coverage) 산출 - 제공된 손익계산서 실제 데이터 사용",
                "6. 배당정책 지속가능성 분석 (Payout Ratio, Dividend Coverage) - 제공된 배당 실제 데이터 활용",
                "7. 계절성/경기민감성이 재무성과에 미치는 영향도 평가",
                "🔴 **데이터 활용 원칙**:",
                "- 1순위: 제공된 DART 재무제표의 실제 수치 사용",
                "- 영업현금흐름, 투자현금흐름, 재무현금흐름은 현금흐름표에서 추출",
                "- 매출, 영업이익, 순이익은 손익계산서에서 추출",
                "- 자산, 부채, 자본은 재무상태표에서 추출",
                "- 실제 데이터 부족시: 가정 사용하되 반드시 '(가정)' 표시하고 근거 설명",
                "- 가정 예시: '영업현금흐름 30조원 (가정: 전년 대비 10% 증가 추정)'",
                "- 실제값과 가정값을 명확히 구분하여 투명성 확보",
            ],
        ...
        valuation_specialist = AnalystAgent(
            ...
            sector_specific_points=[
                ...
                "⭐ 목표가 산출 시 반드시 다음을 포함:",
                "🔴 **중요**: 제공된 실제 재무데이터를 최우선으로 사용하고, 가정 사용시 반드시 '(가정)' 표시",
                "1. DCF 기반 내재가치 계산식과 전제조건 명시 - 제공된 현금흐름표 실제 데이터 기반",
                "2. PER/PBR 멀티플 근거와 업계 비교 데이터 - 제공된 손익계산서, 재무상태표 실제 수치 활용",
                "3. 각 밸류에이션 방법론별 가중평균 산출 과정 - 실제 계산된 값만 사용",
                "4. 목표가 도출을 위한 구체적 수치와 근거 - 모든 수치는 제공된 재무제표에서 추출",
                "5. 시나리오별 목표가 range와 확률 배정",
                "6. 투자의견(매수/보유/매도) 판단 기준 설명",
                "🔴 **밸류에이션 데이터 활용 원칙**:",
                "- FCF 계산: 영업현금흐름 - 자본적지출 (현금흐름표 실제 데이터 우선)",
                "- WACC 계산: 실제 이자비용, 시가총액, 부채 데이터 우선 활용",
                "- 성장률: 과거 3년 실제 매출/이익 성장률 기반 계산",
                "- 멀티플: 실제 순이익, 자기자본, EBITDA 수치 우선 사용",
                "- 실제 데이터 부족시: 가정 사용하되 반드시 '(가정)' 표시하고 근거 설명",
                "- 가정 예시: 'FCF 5조원 (가정: 영업CF 30조 - 자본적지출 25조 추정)'",
