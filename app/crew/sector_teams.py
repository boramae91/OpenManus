...
class SectorTeamFactory:
    ...
    def create_sector_team(self, sector: GICSSector) -> SectorTeam:
        ...
        # 1. 통합 재무분석가 (펀더멘털 + 밸류에이션 통합) - 🎯 시니어 애널리스트 수준
        integrated_financial_analyst = AnalystAgent(
            name=f"{sector_korean_name} 통합 재무분석가",
            role="Integrated Financial Analyst",
            expertise="재무제표 분석, 기업가치 평가, 투자 의견 종합",
            analysis_focus=f"{sector_korean_name} 기업의 재무 건전성, 성장성, 내재가치를 종합적으로 분석",
            key_methods=[
            ...
            sector_context=f"{sector_korean_name} 섹터의 특성을 반영한 통합 재무분석 및 밸류에이션 전문가",
            sector_specific_points=[
                f"통합 재무분석 관점에서 {sector_context.get('valuation_approach', '가치평가 방법')}",
                "⭐ **통합 재무분석 시 반드시 다음을 포함**:",
                "🔴 **중요**: 제공된 실제 재무데이터를 최우선으로 사용하고, 가정 사용시 반드시 '(가정)' 표시",
                "💡 **1단계: 재무 건전성 분석** (CoT 적용)",
                "",
                "**사고 과정**:",
                "1. **데이터 검증**: 3년간 재무제표 데이터의 일관성과 신뢰성 확인",
                "2. **비율 계산**: 핵심 재무비율의 정확한 계산과 트렌드 분석",
                "3. **건전성 평가**: 수익성, 성장성, 안정성의 종합적 평가",
                "4. **리스크 식별**: 재무적 위험요소와 취약점 파악",
                "",
                "**분석 요구사항**:",
                ...
                "1-5. DuPont 분해를 통한 ROE 동력 분석 (순이익률×자산회전율×레버리지)",
                "📊 **2단계: 경쟁사 비교 및 업계 분석** (CoT 적용)",
                "",
                "**사고 과정**:",
                ...
                "2-4. **인터넷 서치로 확인**: 업계 평균 PER, PBR, EV/EBITDA 멀티플과 비교",
                "2-5. **검색 출처 명시**: '인터넷 서치를 통해 확인한 정보는 [출처: 검색결과]로 명시'",
                "🎯 **3단계: 내재가치 산출 (DCF + 멀티플 통합)** (CoT 적용)",
                "",
                "**사고 과정**:",
                "1. **FCF 예측**: 향후 3-5년간 FCF의 현실적 예측",
                "2. **WACC 계산**: 실제 재무데이터 기반 WACC 산출",
                "3. **DCF 계산**: 예측된 FCF와 WACC로 내재가치 산출",
                "4. **멀티플 보정**: 업계 평균과 비교하여 DCF 결과 보정",
                "",
                "**분석 요구사항**:",
                "3-1. **FCF Yield 완전 계산**: FCF/시가총액 × 100, 시가총액은 현재주가 × 발행주식수로 산출",
                "3-2. **WACC 실제 계산**: 사업보고서 데이터로 WACC 직접 산출",
                "   - 타인자본 비용(Rd) = 이자비용 ÷ 유이자부채",
                "   - 법인세율(T) = 법인세비용 ÷ 세전이익",
                "   - 자기자본비용(Re) = 국고채 3년물 + 베타 × 시장위험프리미엄 (인터넷 서치로 베타 확인)",
                "   - WACC = (시가총액/(시가총액+차입금) × Re) + (차입금/(시가총액+차입금) × Rd × (1-T))",
                "3-3. **DCF 모델 상세**: 향후 3년 FCF 예측, 계산된 WACC 활용, Terminal Value 계산",
                "3-4. **멀티플 방법**: Forward PER, PBR 기반 목표가 (업계 평균 적용)",
                "3-5. **Sum-of-Parts**: 주요 사업부문별 밸류에이션과 지분가치 합산",
                "3-6. **인터넷 서치 활용**: 애널리스트 컨센서스 목표가와 비교 분석",
                "⚖️ **4단계: 시나리오별 밸류에이션** (CoT 적용)",
                "",
                "**사고 과정**:",
                "1. **시나리오 구축**: 현실적인 낙관/기본/비관 시나리오 설정",
                "2. **각 시나리오별 분석**: 시나리오별 재무성과와 밸류에이션 계산",
                "3. **확률 가중**: 각 시나리오의 발생 확률을 고려한 확률가중 가치 계산",
                "4. **민감도 분석**: 주요 변수의 변화가 목표가에 미치는 영향 분석",
                "",
                "**분석 요구사항**:",
                "4-1. **낙관 시나리오 (25% 확률)**: 최고 실적 가정시 목표가와 근거",
                "4-2. **기본 시나리오 (50% 확률)**: 컨센서스 기반 목표가와 근거",
                "4-3. **비관 시나리오 (25% 확률)**: 악재 반영시 목표가와 근거",
                "4-4. **확률가중 목표가**: 3시나리오 확률 가중 평균값 산출",
                "4-5. **민감도 분석**: 핵심 변수 ±10% 변동시 목표가 변화폭",
                "4-6. **ESG 요인 반영**: ESG 개선/악화시 밸류에이션 프리미엄/디스카운트",
                "🎯 **5단계: 종합 투자 의견 도출** (CoT 적용)",
                "",
                "**사고 과정**:",
                "1. **종합 분석**: 모든 재무지표와 밸류에이션 결과를 통합 검토",
                "2. **투자 매력도 판단**: 수익성, 성장성, 안정성, 밸류에이션의 균형점 도출",
                "3. **리스크 평가**: 각 리스크 요인의 발생 확률과 영향도 정량화",
                "4. **최종 의견 도출**: 모든 분석을 종합한 명확한 투자 권고",
                "",
                "**분석 요구사항**:",
                "5-1. Working Capital 변동이 영업현금흐름에 미치는 영향 정량화 - 제공된 재무제표 실제 수치 활용",
                "5-2. 부채상환능력 지표 (Interest Coverage, Debt Service Coverage) 산출 - 제공된 손익계산서 실제 데이터 사용",
                "5-3. 배당정책 지속가능성 분석 (Payout Ratio, Dividend Coverage) - 제공된 배당 실제 데이터 활용",
                "5-4. 계절성/경기민감성이 재무성과에 미치는 영향도 평가",
                "5-5. **최종 투자 의견**: 매수/보유/매도 판단과 구체적 근거, 리스크 요인 명시",
                "5-6. **12개월 목표가**: 확률가중 목표가 기반 12개월 목표가 제시",
                "🔴 **데이터 활용 우선순위**:",
        ...
        team = SectorTeam(
            ...
            team_description=(
                ...
                f"섹터 특화 분석 포인트와 리스크 요소를 반영하여 시니어 애널리스트 수준의 분석을 제공합니다."
            ),
            experts=experts,
...
""",
    ...
    def _create_risk_analysis_chain(
        ...
        """
        리스크 평가자를 위한 LangChain Chain 생성 (웹서치 도구 포함)
        """
        try:
            llm = ChatOpenAI(
                model="gpt-4o",
...
""",
    ...
    def _create_industry_analysis_chain(
        ...
        """
        산업 전문가를 위한 LangChain Chain 생성 (웹서치 도구 포함)
        """
        try:
            llm = ChatOpenAI(
                model="gpt-4o",
