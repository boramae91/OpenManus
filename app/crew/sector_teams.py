...
class SectorTeamFactory:
    ...
    def create_sector_team(self, sector: GICSSector) -> SectorTeam:
        ...
        integrated_financial_analyst = AnalystAgent(
            ...
            sector_specific_points=[
                ...
                "",
                "🧠 **PHASE 1: 초기 데이터 검증 및 CoT 분석**",
                "**🔍 Step 1-1: 데이터 무결성 검증 (Chain of Thought)**",
                "```",
                "내 사고 과정:",
                "1. 먼저 3년간 재무제표 데이터를 체계적으로 검토하겠습니다.",
                "2. 각 항목의 일관성을 확인하고, 특이한 변동이 있는지 파악하겠습니다.",
                "3. 데이터 품질에 대한 신뢰도를 평가하겠습니다.",
                "4. 만약 데이터에 문제가 있다면, 그 원인을 분석하고 대안을 제시하겠습니다.",
                "```",
                "**분석 요구사항**:",
                "- 3년간 매출, 영업이익, 순이익, 자산, 부채, 자본의 연속성 검증",
                "- 계정과목 변경, 회계기준 변경, 합병/분할 등 특수 상황 식별",
                "- 데이터 신뢰도 점수 (1-10점) 부여 및 근거 제시",
                "- 문제 발견 시 보정 방법 또는 주의사항 명시",
                "",
                "**🧮 Step 1-2: 핵심 재무비율 CoT 계산**",
                "```",
                "내 사고 과정:",
                "1. ROE 계산: 순이익을 자기자본으로 나누되, 평균 자기자본을 사용해야 하는지 검토",
                "2. ROA 계산: 순이익을 총자산으로 나누되, 이자비용 조정 필요성 판단",
                "3. ROIC 계산: NOPAT을 투하자본으로 나누되, 각 구성요소 정확성 확인",
                "4. 각 비율의 3년 추세를 분석하여 개선/악화 패턴 파악",
                "5. 계산 결과의 합리성을 업계 상식과 비교 검증",
                "```",
                "**분석 요구사항**:",
                "- ROE, ROA, ROIC 정확 계산 (평균 자본 사용, 조정 사항 명시)",
                "- 유동비율, 당좌비율, 부채비율 계산 및 안전성 평가",
                "- 3년 CAGR 계산: 매출, 영업이익, 순이익 (복합성장률 공식 적용)",
                "- 각 비율의 신뢰도 및 계산 과정 투명성 확보",
                "",
                "🔄 **PHASE 2: Self-Critique 및 다각도 분석**",
                "**🤔 Step 2-1: 자기 비판적 경쟁사 분석**",
                "```",
                "내 분석에 대한 자기 비판:",
                "1. 내가 선택한 경쟁사들이 정말 적절한가? 다른 관점에서 볼 때는?",
                "2. 업계 평균과 비교할 때, 내 분석이 편향되지 않았나?",
                "3. 경쟁우위라고 판단한 근거가 충분히 객관적인가?",
                "4. 반대 의견: 만약 경쟁사가 더 우수하다면 그 이유는?",
                "```",
                "**분석 요구사항**:",
                "- **인터넷 서치**: 동종업계 상위 3-5개 경쟁사 최신 재무비율",
                "- Percentile 순위 계산 (업계 내 상대적 위치)",
                "- **반박 관점 검토**: 경쟁사가 우수할 수 있는 영역 분석",
                "- **검증**: 경쟁우위 지속성에 대한 반대 시나리오 검토",
                "- **출처 명시**: [인터넷 서치 출처: 검색결과] 반드시 표기",
                "",
                "**🎯 Step 2-2: Multi-Perspective 현금흐름 분석**",
                "```",
                "다각도 관점에서 현금흐름 분석:",
                "1. 투자자 관점: FCF의 배당 및 성장 투자 여력",
                "2. 경영진 관점: 운영 효율성 및 자본 배분 전략",
                "3. 채권자 관점: 부채 상환 능력 및 재무 안정성",
                "4. 업계 관점: 섹터 특성을 고려한 현금흐름 패턴",
                "```",
                "**분석 요구사항**:",
                "- 3년간 FCF 변동성 분석 (표준편차, 변동계수 계산)",
                "- 영업CF 품질 평가 (Working Capital 변동 영향 분리)",
                "- 투자CF 패턴 분석 (성장 투자 vs 유지 투자 구분)",
                "- 재무CF 분석 (배당, 자사주 매입, 차입 패턴)",
                "",
                "🎯 **PHASE 3: Reasoning-Enhanced 밸류에이션**",
                "**🧠 Step 3-1: WACC 계산 추론 과정**",
                "```",
                "WACC 계산 시 내 추론 과정:",
                "1. 베타 추정: 왜 이 베타가 적절한가? 업계 특성을 반영했는가?",
                "2. 위험프리미엄: 현재 시장 상황을 고려할 때 적정한가?",
                "3. 자본구조: 현재 구조가 최적인가? 목표 구조와 차이는?",
                "4. 세율: 실효세율과 법정세율 중 어느 것이 적절한가?",
                "```",
                "**분석 요구사항**:",
                "- **베타 검색**: 인터넷 서치로 베타 확인 (복수 출처 비교)",
                "- **실효세율 계산**: 법인세비용 ÷ 세전이익 (3년 평균)",
                "- **자본비용 계산**: Re = Rf + β × MRP (각 변수 근거 명시)",
                "- **WACC 산출**: 가중평균 계산 과정 단계별 제시",
                "- **민감도 분석**: 주요 변수 ±10% 변동시 WACC 변화",
                "",
                "**💎 Step 3-2: DCF 모델 Self-Validation**",
                "```",
                "DCF 모델에 대한 자기 검증:",
                "1. 내 FCF 예측이 과도하게 낙관적이지 않은가?",
                "2. 성장률 가정이 업계 성숙도와 일치하는가?",
                "3. Terminal Value 계산이 합리적인가?",
                "4. 전체 결과가 상식적으로 타당한가?",
                "```",
                "**분석 요구사항**:",
                "- **FCF 예측**: 향후 3년 FCF 예측 (보수적/기본/낙관적)",
                "- **성장률 검증**: 과거 성장률 vs 예상 성장률 비교",
                "- **Terminal Value**: 영구성장률 및 Exit Multiple 방법 병행",
                "- **Sanity Check**: 계산 결과의 합리성 검토",
                "",
                "🔄 **PHASE 4: 시나리오별 Confidence Scoring**",
                "**⚖️ Step 4-1: 동적 시나리오 확률 추론**",
                "```",
                "시나리오별 확률 추론 과정:",
                "1. 낙관 시나리오: 어떤 요인들이 이를 뒷받침하는가? 발생 가능성은?",
                "2. 기본 시나리오: 현재 추세가 지속될 근거는? 확률은?",
                "3. 비관 시나리오: 어떤 리스크가 현실화될 수 있는가? 확률은?",
                "4. 각 시나리오의 확률 합계가 100%인지 검증",
                "```",
                "**분석 요구사항**:",
                "- **확률 추론**: 데이터와 시장 상황 기반 각 시나리오 확률 산정",
                "- **근거 제시**: 각 확률의 정량적/정성적 근거 명시",
                "- **확률 검증**: 확률 합계 100% 확인 및 조정",
                "- **민감도**: 확률 변동시 목표가 변화 분석",
                "",
                "**🎯 Step 4-2: 목표가 Confidence Level**",
                "```",
                "목표가 신뢰도 평가:",
                "1. DCF 결과 신뢰도: 입력 변수의 정확성 (1-10점)",
                "2. 멀티플 결과 신뢰도: 비교 대상의 적절성 (1-10점)",
                "3. 시나리오 분석 신뢰도: 가정의 현실성 (1-10점)",
                "4. 종합 신뢰도: 가중평균 계산",
                "```",
                "**분석 요구사항**:",
                "- **신뢰도 점수**: 각 밸류에이션 방법별 신뢰도 (1-10점)",
                "- **가중평균 목표가**: 신뢰도 기반 가중평균 계산",
                "- **신뢰구간**: 목표가의 ±20% 구간 내 확률 제시",
                "- **리스크 조정**: 높은 불확실성 시 할인율 적용",
                "",
                "🧠 **PHASE 5: Final Reflection & 종합 판단**",
                "**🤔 Step 5-1: 전체 분석 Self-Critique**",
                "```",
                "전체 분석에 대한 최종 자기 비판:",
                "1. 내가 놓친 중요한 요소는 없는가?",
                "2. 분석 결과가 일관성이 있는가?",
                "3. 투자자에게 실질적으로 도움이 되는 인사이트인가?",
                "4. 만약 내가 투자자라면 이 분석을 신뢰할 것인가?",
                "```",
                "**분석 요구사항**:",
                "- **Gap Analysis**: 분석에서 부족한 부분 식별",
                "- **일관성 검토**: 각 단계 결과의 논리적 일관성 확인",
                "- **실용성 평가**: 투자 의사결정에 실질적 도움 정도",
                "- **개선 방안**: 분석 품질 향상을 위한 제안사항",
                "",
                "**📊 Step 5-2: 최종 투자 의견 (Multi-Perspective)**",
                "```",
                "최종 의견 도출 과정:",
                "1. 불 케이스 (Bull Case): 가장 긍정적 시나리오와 근거",
                "2. 베어 케이스 (Bear Case): 가장 부정적 시나리오와 근거",
                "3. 베이스 케이스 (Base Case): 가장 가능성 높은 시나리오",
                "4. 균형점 분석: 각 케이스의 확률과 리스크-리턴 분석",
                "```",
                "**분석 요구사항**:",
                "- **Bull/Bear/Base Case**: 각 시나리오별 상세 분석",
                "- **확률 가중 수익률**: 기대수익률 및 리스크 계산",
                "- **투자 권고**: 매수/보유/매도 및 신뢰도 점수",
                "- **Action Items**: 모니터링 포인트 및 재검토 시점",
                "",
                "🎯 **고급 분석 기법 적용**:",
                "**🔍 데이터 활용 우선순위 (AI 강화)**:",
                "- 1순위: 제공된 DART 재무제표 + AI 검증",
                "- 2순위: 인터넷 서치 + 교차 검증",
                "- 3순위: 합리적 가정 + 신뢰도 점수",
                "- **AI 검증**: 각 데이터 포인트의 신뢰도 평가",
                "",
                ...
                "",
                "**📈 출력 품질 기준**:",
                "- 모든 비율: 소수점 둘째 자리 + 신뢰도 점수",
                "- 계산 과정: 단계별 명시 + 검증 과정",
                "- 비교 분석: 구체적 수치 + 통계적 유의성",
                "- 결론: 정량적 근거 + 신뢰구간 제시",
            ],
    ...
    def _create_integrated_financial_analysis_chain(
        ...
        try:
            ...
            # 시스템 프롬프트 생성 (AgentExecutor 호환) - 6가지 핵심 전략 반영
            system_prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "system",
...
**✅ 1. Chain of Thought 기반 추론 템플릿**
모든 분석에서 다음 3단계 추론을 반드시 수행하세요:
1. 이 수치가 무엇을 의미하는가? (정의 및 계산)
2. 경쟁사 또는 과거와 비교해 유의미한 차이가 있는가? (상대적 위치)
3. 이 차이가 투자 판단에 어떤 영향을 미치는가? (투자 시사점)

**✅ 2. 논리구조 틀 적용**
각 지표별로 다음 구조로 해석하세요:
[지표 해석 구조]
1. 정의 및 수식: 해당 지표의 정의와 계산 방법
2. 현재 수치 및 의미: 계산된 수치와 그 의미
3. 경쟁사/과거비교: 업계 평균, 경쟁사, 과거 트렌드와 비교
4. 원인 분석: 수치 변화의 근본 원인 (업황, 전략, 구조적 요인)
5. 투자 시사점: 투자 판단에 미치는 영향

**✅ 3. 산업 구조 및 동향과의 연결 강화**
웹 검색을 통해 다음 정보를 반드시 확인하고 분석에 반영하세요:
- 최신 산업 리포트 및 업황 동향
- 경쟁사 CapEx/전략 변화
- 섹터별 주요 이슈 및 정책 변화
- 글로벌 시장 동향과의 연관성
...
**✅ 4. 리스크 시나리오 분석 자동화**
모든 분석 결과에 기반하여 다음 3가지 시나리오를 제시하세요:
[시나리오 분석 구조]
1. 기준 시나리오: 기본 가정 + 결과
2. 낙관 시나리오: 매출 성장률 +2%, WACC ↓ → 결과
3. 비관 시나리오: 매출 ↓, 마진 ↓, CapEx ↑ → 결과 및 리스크 요인

**✅ 5. 통합 투자 의견 도출 템플릿**
최종 투자 의견은 다음 구조로 작성하세요:
[종합 투자 의견]
- 요약: 현재 펀더멘털, 밸류에이션, 업황 판단 (한 문단)
- 핵심 근거 3가지: 가장 중요한 분석 결과
- 리스크 요인 및 대응 방안: 주요 위험 요소와 대응책
- 최종 판단: 매수/중립/매도 + 목표가 + 투자 기간

**✅ 6. 분석 결과의 평가 지표**
분석 완료 후 다음 항목으로 자체 평가하세요:
- 해석 깊이: 수치가 의미하는 바를 산업/경쟁/비즈니스로 연결했는가?
- 논리 일관성: 해석 내용과 수치/산출근거 간 논리 흐름이 일치하는가?
- 투자 판단 근거: 투자 의견에 수치/사실 기반 논리가 존재하는가?

**📊 5단계 분석 프로세스 (Chain of Thought 적용)**

**1단계: 재무 건전성 분석**
- 핵심 재무비율 계산 (ROE, ROA, ROIC, 유동비율, 부채비율)
- 각 비율별 Chain of Thought 적용:
  * 이 비율이 무엇을 의미하는가?
  * 업계 평균/경쟁사 대비 어떤 위치인가?
  * 이 수준이 투자 판단에 어떤 영향을 미치는가?
- 3년간 트렌드 분석 및 변화 패턴 식별
- DuPont 분석을 통한 ROE 분해 (영업이익률 × 자산회전율 × 재무레버리지)
- 현금흐름 안정성 평가 (FCF/영업이익, 영업현금흐름/총부채)
...
- 가중평균 목표가 계산 (각 방법론별 신뢰도 반영)

**4단계: 시나리오별 밸류에이션**
- 낙관/기본/비관 3시나리오 분석
- 주요 변수별 민감도 분석 (±10%, ±20% 변동)
- 확률가중 목표가 산출
...
**🔍 웹 검색 활용 가이드**
- 최신 업계 리포트 및 애널리스트 의견
- 경쟁사 실적 발표 및 전략 변화
- 섹터별 정책 및 규제 동향
- 글로벌 시장 동향과의 연관성

모든 분석에서 Chain of Thought를 적용하고, 논리구조 틀을 준수하며, 웹 검색을 통해 최신 정보를 반영하세요.
""",
                    ),
                    MessagesPlaceholder(variable_name="chat_history"),
                    ("human", "{input}"),
...
사용자의 요청을 바탕으로 종합적인 기술적 분석을 수행하세요.
