...
🎯 **시니어 애널리스트 분석 지침 (ChatGPT 6가지 핵심 개선사항 완전 반영)**:
위의 데이터를 바탕으로 실제 증권사 시니어 애널리스트 수준의 전문적이고 정량적인 분석을 수행해주세요.

📊 **필수 분석 단계별 가이드**:

**1. 해석의 깊이 강화 - '왜?' 분석 필수**:
- 모든 재무지표에 대해 "왜 이런 수치가 나왔는지" 원인 분석 필수
- 예시: "ROE가 9.2%인 이유는 순이익률 11%가 양호하지만 자산회전율 0.64가 낮기 때문"
- 이상 수치 발견 시 반드시 원인 진단 (예: ROIC 2.69%가 낮은 이유는 대규모 신규투자로 투하자본 증가?)
- 🌐 **웹서치 활용**: 특이사항 발견 시 최신 뉴스/공시를 검색하여 배경 파악

**2. 경쟁사 비교 분석 필수 (절대적 핵심)**:
- 🌐 **웹서치 필수**: 동종업계 상위 3~5개 경쟁사의 최신 재무비율 검색
- 업계 평균 대비 Percentile 순위 제시 (ROE, ROIC, 매출성장률, 이익률, PER, PBR, EV/EBITDA)
- 🌐 **웹서치**: 업계 평균 멀티플과 WACC 범위 확인
- **경쟁사 비교표 작성**: "당사 vs 경쟁사A vs 경쟁사B vs 업계평균" 형태로 정리
- **상대적 포지셔닝**: "업계 내 상위 25% 수준" 등 구체적 위치 제시

**3. 시계열 분석 의무화 (3-5년 트렌드)**:
- 🌐 **웹서치**: 과거 3-5년간 주요 재무비율 데이터 검색
- ROE, ROIC, 매출성장률, 영업이익률의 연도별 추이 그래프 표현
- CAGR 계산: 매출/영업이익/순이익의 3년 복합성장률 산출
- **트렌드 해석**: 상승/하락/횡보 구분 및 향후 지속성 평가
- FCF 안정성: 3년간 FCF 변동성과 영업CF 대비 투자CF 비율 분석

**4. 올바른 FCF 계산 및 밸류에이션 (ChatGPT 지적 핵심)**:
- **FCF 정의 수정**: FCF = 영업활동현금흐름 - 자본적지출 (재무활동현금흐름 아님!)
- FCF Yield = FCF / 시가총액 × 100 (시가총액 = 현재주가 × 발행주식수)
- **WACC 실제 계산**:
  * 타인자본비용 = 이자비용 ÷ 유이자부채
  * 법인세율 = 법인세 ÷ 세전이익
  * 🌐 **베타 검색**: 해당 종목의 베타 계수를 웹서치로 확인
  * 자기자본비용 = 국고채수익률 + 베타 × 위험프리미엄
  * WACC = (시총/(시총+차입금) × 자기자본비용) + (차입금/(시총+차입금) × 타인자본비용 × (1-세율))
- **ROIC vs WACC 비교**: 가치창출 여부 명확히 판단
- 🌐 **애널리스트 컨센서스**: 목표가 검색하여 밸류에이션 검증

**5. 질적 분석 및 산업 분석 추가**:
- 🌐 **업황 분석**: 최근 산업 동향, 업사이클 위치, 규제 변화 검색
- **사업보고서 활용**: 제공된 사업보고서에서 사업전략, 위험요인, 투자계획 분석
- **분기보고서 활용**: 최근 실적 변화의 질적 요인 파악
- ESG 요인이 재무성과에 미치는 영향 분석
- 🌐 **거시경제 영향**: 환율, 금리, 원자재 가격 등이 당사에 미치는 영향 검색

**6. 사업보고서/분기보고서 딕셔너리 최대 활용**:
- **재무제표 세부분석**: 딕셔너리에서 손익계산서, 재무상태표, 현금흐름표 상세 분석
- **주석사항 검토**: 회계정책 변경, 특수관계자 거래, 우발부채 등 확인
- **경영진 전망**: 사업보고서의 전망 섹션과 실제 실적 비교 분석
- **부문별 분석**: 사업부문별 매출/이익 기여도 및 성장성 평가
- **정보 부족 시 웹서치**: 딕셔너리에 없는 정보는 즉시 웹검색으로 보완

**📋 분석 결과 필수 포함 사항**:

**A. 계산 과정 투명성**:
- 모든 비율을 소수점 둘째 자리까지 표시
- 계산 과정 단계별 명기 (예: ROE = 순이익(2.36조) ÷ 자기자본(23.4조) = 10.09%)
- 가정과 추정치 명확히 구분
...
**C. 리스크 요인**:
- 업종별 특화 리스크 (예: 반도체-업황 사이클, 금융-금리 리스크)
- ESG 리스크와 기회요인
- 지정학적/규제 리스크

**D. 목표주가 및 투자의견**:
- 보수적/기본/낙관적 시나리오별 12개월 목표가
- 매수/보유/매도 명확한 투자의견
- 투자포인트 3가지와 리스크 3가지

**🌐 웹서치 활용 체크리스트**:
✅ 동종업계 경쟁사 재무비율 (필수)
✅ 업계 평균 PER, PBR, EV/EBITDA (필수)
✅ 애널리스트 컨센서스 목표가 (필수)
✅ 최근 업황 및 산업 동향 (필수)
✅ 해당 종목의 베타 계수 (필수)
✅ 과거 3-5년 재무데이터 (필수)
...
**🔍 웹서치 결과 표기법**:
- 모든 웹검색 정보는 '[웹서치 출처: 검색결과]'로 명시
- 검색 실패 시 '(웹서치 불가: 합리적 가정 사용)'으로 표기
- 여러 출처 종합 시 '복수 출처 종합' 명시

**💡 분석 품질 최종 검증**:
- 모든 주장에 정량적 근거 존재 확인
- 과거 데이터와 경쟁사 비교 완료 확인
- FCF 정의 올바른 사용 확인
- WACC 계산 완료 및 ROIC 비교 확인
- 질적 요인과 정량적 요인 균형 확인
- 실무진 즉시 활용 가능한 구체성 확인

**한국어로 작성하되, 초보자도 이해할 수 있게 쉬운 표현과 비유를 사용하여 전체적인 설명을 해주세요. 각 재무지표가 무엇을 의미하는지, 왜 중요한지 설명하고, 복잡한 개념은 일상생활의 예시로 비교 설명해주세요.**

전문가로서의 깊이 있는 인사이트와 즉시 실행 가능한 투자 판단을 제공해주세요.
"""
