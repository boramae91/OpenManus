...
🎯 **시니어 애널리스트 분석 지침 (ChatGPT 6가지 개선사항 반영)**:
위의 데이터를 바탕으로 실제 증권사 시니어 애널리스트 수준의 전문적이고 정량적인 분석을 수행해주세요.

📊 **ChatGPT 지적사항 해결을 위한 필수 포함 요소**:

**1. 시계열 분석 (지적사항 1,2 해결)**:
- 과거 3년간 주요 재무비율 트렌드 (ROE, ROA, ROIC, 유동비율, 부채비율)
- 매출/영업이익/순이익의 3년 CAGR 계산 및 성장 지속성 평가
- 현금흐름 안정성 (3년간 FCF 변동성과 영업CF 대비 투자CF 비율)

**2. 경쟁사 비교 및 벤치마크 분석 (지적사항 3 해결)**:
- 🌐 **인터넷 서치 필수**: 동종업계 상위 3~5개 경쟁사의 최신 재무비율 검색
- 업계 평균 대비 Percentile 순위 제시 (ROE, ROIC, 매출성장률, 이익률)
- 🌐 **인터넷 서치**: 업계 평균 PER, PBR, EV/EBITDA 멀티플 확인
- **검색 결과 명시**: '[인터넷 서치 출처: 검색결과]'로 표기

**3. 이상 변화 설명 및 질적 분석 (지적사항 4,6 해결)**:
- 재무지표 이상변화 원인 분석 (매출 감소인데 이익 증가 등)
- 🌐 **인터넷 서치**: 최근 업황, 산업 사이클, 규제 변화 검색
- ESG 요인이 재무성과에 미치는 영향
- 🌐 **인터넷 서치**: 지정학적 리스크, 환율, 원자재 가격 영향 확인

**4. 완성된 밸류에이션 분석 (지적사항 5 해결)**:
- FCF Yield 완전 계산: FCF/시가총액 × 100 (시가총액 = 현재주가 × 발행주식수)
- 멀티플 분석: 현재 PER, PBR, EV/EBITDA와 과거 3년 평균 비교
- 🌐 **인터넷 서치**: 애널리스트 컨센서스 목표가 검색하여 비교
- **WACC 실제 계산**: 사업보고서 데이터로 WACC 직접 산출 후 DCF 계산
  * 타인자본비용 = 이자비용÷유이자부채, 법인세율 = 법인세÷세전이익
  * 자기자본비용 = 국고채수익률 + 베타×위험프리미엄 (베타는 인터넷 서치)
  * WACC = (시총/(시총+차입금)×자기자본비용) + (차입금/(시총+차입금)×타인자본비용×(1-세율))
- 목표주가 레인지: 보수적/기본/낙관적 시나리오별 12개월 목표가

**5. 구체적 수치와 계산 과정**:
- 모든 비율은 소수점 둘째 자리까지 표시
- 계산 과정 단계별 명시 (예: ROE = 순이익(2.3조) ÷ 자기자본(25조) = 9.2%)
- 업계 평균과의 괴리를 구체적 수치로 제시

**6. 최종 투자 의견**:
- 매수/보유/매도 판단과 구체적 근거
- 핵심 리스크 요인 명시
- 실무진이 즉시 활용 가능한 구체적 권고

**🌐 인터넷 서치 활용 가이드**:
- 필수 검색 항목: 경쟁사 재무비율, 업계 평균 멀티플, 애널리스트 컨센서스, 최신 업황
- 모든 인터넷 검색 정보는 '[인터넷 서치 출처: 검색결과]'로 명시
- 검색이 실패하거나 정보가 부족한 경우 '(인터넷 서치 불가: 가정 사용)'으로 표기
