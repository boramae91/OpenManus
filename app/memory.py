...
from app.schema import Memory as BaseMemory
...
class ValidatedMemory(BaseMemory):
    ...
    def _validate_tool_message(self, message: Message) -> None:
        """tool 메시지가 유효한지 검사 (개선된 버전)"""
        if not message.tool_call_id:
            # tool_call_id가 없는 경우는 정상적인 상황일 수 있음 (선택적 경고)
            return
            ...
            if prev_msg.role == "assistant" and prev_msg.tool_calls:
                for tool_call in prev_msg.tool_calls:
                    if (
                        ...
                        and tool_call.get("id") == message.tool_call_id
                    ):
