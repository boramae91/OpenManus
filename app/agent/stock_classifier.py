...
"""
    ...
    def extract_stock_info_from_analysis(self, analysis_text: str) -> Dict:
        ...
        try:
            # 패턴 1: "종목명: XXX" 형태 (JSON에서 발견된 패턴 포함)
            name_patterns = [
                r"종목명\s*[:：]\s*([가-힣A-Za-z0-9\s&\.\-]+)(?=\s*(?:\(|$|\n|,))",
                r"회사명\s*[:：]\s*([가-힣A-Za-z0-9\s&\.\-]+)(?=\s*(?:\(|$|\n|,))",
                r"기업명\s*[:：]\s*([가-힣A-Za-z0-9\s&\.\-]+)(?=\s*(?:\(|$|\n|,))",
                # JSON에서 발견된 패턴: "**종목명 및 종목코드**: 한화오션 (종목코드: 042660)"
                r"\*\*종목명[^:]*\*\*:\s*([가-힣A-Za-z0-9\s&\.\-]+)(?=\s*\()",
                r"종목명\s*및\s*종목코드\**\s*[:：]\s*([가-힣A-Za-z0-9\s&\.\-]+)(?=\s*\()",
            ]

            # 패턴 2: "종목코드: XXXXXX" 형태 (JSON에서 발견된 패턴 포함)
            code_patterns = [
