...
def extract_stock_name_from_response(response_text, fallback_prompt=""):
    """
    AI Flow 응답 결과에서 실제로 분석된 종목명을 추출하는 함수예요
    ...
    for stock in korean_stocks:
        if stock.upper() in response_upper:
            ...
            logger.info(
                f"Found stock name in Flow response: {stock} -> {extracted_name}"
            )
            ...
            for match in matches:
                ...
                if (
                    ...
                    if any("\uac00" <= char <= "\ud7a3" for char in match):
                        ...
                        logger.info(
                            f"Found company name in Flow response: {match} -> {converted}"
                        )
                        return converted
                    else:
                        # 영어인 경우 그대로 대문자로
                        ...
                        logger.info(
                            f"Found company name in Flow response: {match} -> {extracted}"
                        )
                        return extracted
    ...
    if code_matches:
        ...
        logger.info(
            f"Found stock code in Flow response: {code_matches[0]} -> {code_name}"
        )
        return code_name
    ...
    for match in english_matches:
        if match.upper() not in exclude_english:
            logger.info(f"Found English company name in Flow response: {match}")
            return match.upper()

    # 모든 방법으로도 찾지 못하면 원본 프롬프트에서 추출 시도
    logger.info(
        "No stock name found in Flow response, falling back to prompt extraction"
    )
    return extract_stock_name(fallback_prompt)
...
async def run_flow():
    ...
    try:
        ...
        json_filename = generate_json_filename(stock_name, results_dir, "cancelled")
