...
def interactive_test_advanced():
    ...
    print("\n=== 고급 자동 분류 감지 테스트 ===")
    print("프롬프트를 입력하면 자동 종목 분류 감지 여부를 상세히 분석해드려요!")
    ...
    while True:
        ...
        # 종목 감지
        detected_stock_info, detected_stock_code = extract_stock_name_simple(user_input)
        has_stock_detected = (
            detected_stock_info != "GENERAL" and detected_stock_code is not None
        )
        ...
        print("📊 분석 결과:")
        print(
            f"  키워드 감지: {'✅ 감지됨' if is_keyword_detected else '❌ 감지 안됨'}"
        )
        if found_keywords:
            print(f"    └ 키워드: {', '.join(found_keywords)}")

        print(f"  종목 감지: {'✅ 감지됨' if has_stock_detected else '❌ 감지 안됨'}")
        if has_stock_detected:
            print(f"    └ 종목: {detected_stock_info} (코드: {detected_stock_code})")

        print(
            f"\n🤖 최종 결과: Stock Classifier {'✅ 실행됨' if is_classification_request else '❌ 실행 안됨'}"
        )

        if is_classification_request:
            print("🏷️ 종목 분류 분석을 자동 실행합니다...")
            if is_keyword_detected and has_stock_detected:
                print("   (키워드 + 종목 둘 다 감지)")
            elif is_keyword_detected:
                print("   (키워드 감지)")
