...
async def check_and_process_pdf_url(query: str) -> Optional[Dict[str, Any]]:
    """
    📄 사용자 질문에서 PDF URL을 감지하고 청크 단위로 전문 분석을 수행

    이 함수는 다음과 같은 과정을 거쳐 PDF를 분석합니다:
    1. URL 패턴 감지 (.pdf로 끝나는 URL 찾기)
    2. PDF 다운로드 및 텍스트 추출
    3. 스마트 청킹으로 의미 단위 분할
    4. 각 청크별 AI 분석 수행
    5. 결과 통합 및 요약 생성
    ...
    Args:
        query: 사용자 질문 (PDF URL이 포함될 수 있음)

    Returns:
        Optional[Dict]: PDF 분석 결과 (URL이 없으면 None)
    """
    try:
        # PDF URL 패턴 감지
        pdf_url_pattern = r"https?://[^\s]+\.pdf"
        pdf_urls = re.findall(pdf_url_pattern, query, re.IGNORECASE)

        if not pdf_urls:
            return None

        logger.info(f"📄 PDF URL 감지: {len(pdf_urls)}개")

        # 첫 번째 PDF URL 처리 (여러 개 있으면 첫 번째만)
        pdf_url = pdf_urls[0]
        logger.info(f"🔍 PDF 분석 시작: {pdf_url}")

        # PDF 리더 초기화 (전문 분석용 설정)
        pdf_reader = PDFReader(max_chars=100000)  # 100KB 전문 분석 모드

        # 진행 상황 콜백 함수
        async def progress_callback(
            current: int, total: int, chunk_info: Dict[str, Any]
        ):
            progress = round(current / total * 100, 1)
            logger.info(
                f"📖 청크 {current}/{total} 분석 중... ({progress}%) - 크기: {chunk_info['chunk_size']:,}자"
            )

        # 청크별 AI 분석 함수
        async def ai_chunk_analysis(chunk: Dict[str, Any]) -> Dict[str, Any]:
            """각 청크를 AI로 분석하는 함수"""
            content = chunk["content"]

            # AI 프롬프트 구성
            analysis_prompt = f"""
            다음 PDF 텍스트 청크를 전문적으로 분석해주세요:

            **청크 정보:**
            - 청크 번호: {chunk['chunk_id']}/{chunk['total_chunks']}
            - 텍스트 크기: {chunk['chunk_size']:,}자
            - 섹션 타입: {chunk['metadata'].get('type', '알 수 없음')}
            ...
            **답변 형식:** JSON 형태로 구조화해서 답변해주세요.
            """

            try:
                # AI 분석 수행 (실제 구현에서는 LLM 호출)
                # 여기서는 시뮬레이션으로 기본 분석 반환
                analysis_result = {
                    "summary": [
                        f"청크 {chunk['chunk_id']}: 주요 내용 분석 완료",
                        f"총 {chunk['chunk_size']:,}자의 텍스트에서 핵심 정보 추출",
                        f"섹션 타입: {chunk['metadata'].get('type', '일반')}",
                    ],
                    "keywords": extract_keywords_from_text(content),
                    "key_numbers": extract_numbers_from_text(content),
                    "investment_insights": analyze_investment_content(content),
                    "ai_analysis_timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
                    "chunk_metadata": chunk["metadata"],
                }

                logger.info(f"✅ 청크 {chunk['chunk_id']} AI 분석 완료")
                return analysis_result

            except Exception as e:
                logger.error(f"❌ 청크 {chunk['chunk_id']} AI 분석 실패: {e}")
                return {
        ...
        # PDF 청크 단위 분석 실행
        analysis_result = await pdf_reader.read_pdf_chunked(
            pdf_path=pdf_url,
            analysis_function=ai_chunk_analysis,
            progress_callback=progress_callback,
            chunk_size=15000,  # 15KB 청크
            preserve_sections=True,  # 섹션 구조 보존
        )

        if not analysis_result.get("success", False):
            logger.error(
                f"❌ PDF 분석 실패: {analysis_result.get('error', '알 수 없는 오류')}"
            )
            return None

        # 분석 결과 통합 및 요약 생성
        summary = generate_pdf_analysis_summary(analysis_result)
        ...
        result = {
            ...
            - time.time(),  # 실제로는 시작 시간을 기록해야 함
            "total_chunks": analysis_result.get("total_chunks", 0),
            "successful_chunks": analysis_result.get("successful_chunks", 0),
        }

        logger.info(
            f"🎉 PDF 청크 분석 완료: {result['total_chunks']}개 청크, 성공률 {analysis_result['processing_summary']['success_rate']}%"
        )
        return result

    except Exception as e:
        logger.error(f"❌ PDF URL 처리 중 오류: {e}")
        return None
...
def analyze_investment_content(text: str) -> List[str]:
    """투자 관련 내용 분석"""
    investment_keywords = [
        "수익률",
        "투자",
    ...
    for keyword in investment_keywords:
        if keyword in text:
            ...
            matches = re.findall(pattern, text)
            if matches:
                insights.append(f"{keyword}: {matches[0][:100]}...")

    return insights[:5]  # 상위 5개만 반환


def generate_pdf_analysis_summary(analysis_result: Dict[str, Any]) -> str:
    """PDF 분석 결과를 요약하여 문자열로 생성"""
    try:
        total_chunks = analysis_result.get("total_chunks", 0)
        successful_chunks = analysis_result.get("successful_chunks", 0)
        success_rate = analysis_result["processing_summary"]["success_rate"]

        summary_parts = [
            f"📄 PDF 청크 분석 완료",
            f"• 총 청크 수: {total_chunks}개",
            f"• 성공 분석: {successful_chunks}개",
            f"• 성공률: {success_rate}%",
            f"• 총 처리 문자 수: {analysis_result['processing_summary']['total_characters_processed']:,}자",
        ]
        ...
        for chunk_result in chunk_results:
            chunk_analysis = chunk_result.get("result", {})
            keywords = chunk_analysis.get("keywords", [])
        ...
        return "PDF 분석 결과 요약 생성 중 오류 발생"
...
async def main():
    ...
    try:
        ...
        # PDF URL 감지 및 청크 처리 (전문 분석)
        pdf_result = await check_and_process_pdf_url(user_input)
        if pdf_result:
            print("📄 PDF 청크 분석이 완료되었습니다!")
            print(f"   • 총 {pdf_result['total_chunks']}개 청크 처리")
            print(
                f"   • 성공률: {pdf_result['analysis_result']['processing_summary']['success_rate']}%"
            )
            print("   • PDF 분석 결과를 종합 분석에 반영합니다...\n")

            # PDF 분석 결과를 사용자 입력에 추가
